...
class GaussianTrainer(object):
    ...
    def setup_depth_predictor(self,):
        ...
        if self.depth_model_type == "zoe":
            ...
            encoder = 'vits'
            size_map = {'vits': "Small", 'vitb': "Base", 'vitl': "Large", 'vitg': "Giant"}
            model_configs = {
            ...
            size = size_map[encoder]
            url = f"https://huggingface.co/depth-anything/Depth-Anything-V2-{size}/resolve/main/depth_anything_v2_{encoder}.pth?download=true"
            depth_anything = DepthAnythingV2(**model_config).eval()
            depth_anything.load_state_dict(torch.hub.load_state_dict_from_url(url))
    ...
    def predict_depth(self, img):
        if self.depth_model_type == "zoe":
            depth = self.depth_model.infer_pil(Image.fromarray(img.astype(np.uint8)),
                                               output_type='tensor')
        elif self.depth_model_type in ["depth_anything", "depth_anything_v2"]:
